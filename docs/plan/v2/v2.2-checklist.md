...
- [ ] Locate `list_documents_paginated()` method
- [ ] Change return type: `Dict[str, Any]` → `DocumentListResponse`
- [ ] Update method implementation:
  - [ ] Create `PaginationMeta` instance
  - [ ] Calculate `has_more` flag
  - [ ] Return `DocumentListResponse` instance
- [ ] Update docstring with new return type
- [ ] Import `DocumentListResponse` and `PaginationMeta`
...
- [ ] Locate `search_documents()` or equivalent method
- [ ] Change return type: `Dict[str, Any]` → `SearchResponse`
- [ ] Update method implementation:
  - [ ] Create `PaginationMeta` instance
...
- [ ] Locate `get_document_details()` method
- [ ] Change return type: `Dict[str, Any]` → `DocumentDetails`
- [ ] Update method implementation:
  - [ ] Return `DocumentDetails` instance
  - [ ] Calculate `version_count`
...
- [ ] Locate `_extract_file_info()` method
- [ ] Update string handling logic:
  - [ ] Import `Path` from `pathlib`
  - [ ] For `str` input, check if `Path(file_input).exists()`
  - [ ] If exists and is file: read file content
  - [ ] If not exists: treat as text content
  - [ ] Use UTF-8 encoding for text content
  - [ ] Set default filename for text content: `"document.txt"`
- [ ] Update docstring to document new behavior
- [ ] Add code comments explaining detection logic
- [ ] Handle edge cases:
  - [ ] Empty strings
  - [ ] Strings that look like paths but don't exist
...
- [ ] Locate `get_permissions_detailed()` or equivalent
- [ ] Change return type: `Dict[str, Any]` → `PermissionListResponse`
- [ ] Update implementation:
  - [ ] Return `PermissionListResponse` instance
  - [ ] Include metadata (requested_by, requested_at)
...
- [ ] Run mypy on service layer files
- [ ] Fix any type checking errors
- [ ] Ensure all return types match method signatures
- [ ] Verify model instantiation is correct

### Phase 2 Completion Criteria

- [ ] All service methods return proper models
- [ ] String detection works for file paths and text content
- [ ] Service layer type hints are complete
- [ ] Mypy passes on service layer
- [ ] No runtime errors on model instantiation

**Phase 2 Sign-off**: ___________ (Date) ___________ (Initials)
...
- [ ] Locate `list_docs()` method
- [ ] Change return type: `Dict[str, Any]` → `DocumentListResponse`
- [ ] Update docstring:
  - [ ] Document return type as `DocumentListResponse`
  - [ ] Add example of accessing response fields
  - [ ] Update `Returns` section
- [ ] Verify service call returns correct type
...
- [ ] Update docstring:
  - [ ] Document return type
  - [ ] Document optional fields (versions, permissions)
  - [ ] Update `Returns` section
...
- [ ] Locate `set_permissions()` method
- [ ] Change parameter type:
  - [ ] Remove `| List[dict]` from `permissions` parameter
  - [ ] Keep only `List[PermissionGrant]`
- [ ] Update return type:
  - [ ] Change `List[Any]` → `List[DocumentACL]`
- [ ] Remove dict-to-model conversion logic:
  - [ ] Find code that checks `isinstance(permissions[0], dict)`
  - [ ] Remove dict conversion code
  - [ ] Simplify implementation
- [ ] Update docstring:
  - [ ] Remove mention of dict support
  - [ ] Document model-only input
  - [ ] Show `PermissionGrant` example
  - [ ] Update `Returns` section
...
- [ ] Locate `transfer_ownership()` method
- [ ] Add explicit return type: `OwnershipTransferResponse`
- [ ] Verify service call returns `OwnershipTransferResponse`
- [ ] Update docstring:
  - [ ] Document return type
  - [ ] Add example
...
- [ ] Locate `_resolve_external_ids()` method
- [ ] Delete entire method
- [ ] Find all usages of `_resolve_external_ids()`:
  - [ ] Search for `_resolve_external_ids` in file
  - [ ] Should find 4 usages in: download, update_metadata, delete, restore_version
- [ ] Remove all calls to `_resolve_external_ids()`
...
- [ ] Locate `download()` method
- [ ] Change `agent_id` type: `str` → `str | UUID`
- [ ] Remove `_resolve_external_ids()` call
- [ ] Pass `agent_id` directly to service (service handles UUID conversion)
- [ ] Update docstring:
  - [ ] Document `agent_id: str | UUID`
  - [ ] Add note about automatic UUID conversion
...
- [ ] Locate `update_metadata()` method
- [ ] Change `agent_id` type: `str` → `str | UUID`
- [ ] Remove `_resolve_external_ids()` call
- [ ] Pass `agent_id` directly to service
- [ ] Update docstring
...
- [ ] Locate `upload()` method docstring
- [ ] Update `file_input` parameter documentation:
  - [ ] Clarify text content detection
  - [ ] Add example: "Hello, world!" → treated as text
  - [ ] Add example: "/path/to/file.txt" → treated as file path if exists
...
- [ ] Verify no `Dict[str, Any]` or `List[Any]` remain in public API
...
- [ ] `set_permissions()` only accepts `List[PermissionGrant]`
- [ ] `_resolve_external_ids()` removed
- [ ] Four methods accept `str | UUID` for agent_id
- [ ] Mypy passes on core.py
- [ ] All docstrings updated

**Phase 3 Sign-off**: ___________ (Date) ___________ (Initials)
...
| Phase 2: Service Layer | ⏳ Not Started | ___ | ___ | 3 days |
| Phase 3: Core SDK | ⏳ Not Started | ___ | ___ | 2 days |
...
- [ ] No `Dict[str, Any]` or `List[Any]` in public API
- [ ] All response models created and tested
- [ ] Smart string detection works for upload
- [ ] set_permissions() is model-only
- [ ] _resolve_external_ids() removed
