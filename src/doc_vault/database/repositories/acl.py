...
class ACLRepository(BaseRepository[DocumentACL]):
    ...
    async def get_all_permissions(
        ...
        """
        Get all permissions for a document grouped by agent (v2.0).

        Args:
            document_id: Document UUID or string

        Returns:
            Dict mapping agent_id to list of permission strings
        ...
        try:
            ...
            query = """
                SELECT agent_id, permission FROM document_acl
                WHERE document_id = $1
                ORDER BY agent_id, permission
            """
            result = await self.db_manager.execute(query, [document_id])
            rows = result.result()
            ...
            for row in rows:
                ...
                permissions[agent_id].append(perm)

            return permissions

        except Exception as e:
            logger.error(
                f"Failed to get all permissions for document {document_id}: {e}"
            )
            from doc_vault.exceptions import DatabaseError

            raise DatabaseError("Failed to get all permissions") from e
    ...
    async def set_permissions(
        ...
        """
        Set permissions for a document (bulk update) (v2.0).
        ...
        Args:
            ...
            permissions: List of permission dicts with keys:
                        - agent_id: Agent UUID
                        - permission: Permission type (READ, WRITE, DELETE, SHARE, ADMIN)
                        - expires_at: Optional expiration datetime
            granted_by: Agent UUID granting the permissions

        Returns:
            List of created DocumentACL instances
        ...
        try:
            ...
            # Delete all existing permissions for this document
            delete_query = "DELETE FROM document_acl WHERE document_id = $1"
            await self.db_manager.execute(delete_query, [document_id])

            # Insert new permissions
            created_acls: List[DocumentACL] = []

            for perm_data in permissions:
                agent_id = self._ensure_uuid(perm_data.get("agent_id"))
                permission = perm_data.get("permission")
                expires_at = perm_data.get("expires_at")

                if not permission:
                    raise ValueError("Permission type must be specified")
                ...
                created = await self.create_from_create_schema(
                    DocumentACLCreate(
                        document_id=document_id,
                        agent_id=agent_id,
                        permission=permission,
